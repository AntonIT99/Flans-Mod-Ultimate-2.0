...
{
    private final ModelRendererTurbo[] spinnerModel;
    private final Vector3f spinnerOrigin;
    ...
    @Override
    public void renderCustom(PoseStack poseStack, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, float scale, GunAnimations anims)
    {
        poseStack.pushPose();
        ...
        poseStack.translate(-spinnerOrigin.x, -spinnerOrigin.y, -spinnerOrigin.z);
        render(spinnerModel, poseStack, buffer, packedLight, packedOverlay, red, green, blue, alpha, scale);
        poseStack.popPose();
    }
}
