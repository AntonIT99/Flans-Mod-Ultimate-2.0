...
public class TmtTessellator extends Tessellator
{
     ...
     */
    private static ByteBuffer byteBuffer = ByteBuffer.allocateDirect(nativeBufferSize * 4);
    ...
    static
    {
        ...
        useVBO = tryVBO && GL.getCapabilities().GL_ARB_vertex_buffer_object;

        if(useVBO)
        {
            vertexBuffers = MemoryUtil.memAllocInt(vboCount);
        ...
        else
        {
            ...
            while(offs < vertexCount)
            {
                ...
                if(TmtTessellator.useVBO)
                {
                    ...
                    GL15.glBindBuffer(ARBVertexBufferObject.GL_ARRAY_BUFFER_ARB, TmtTessellator.vertexBuffers.get(this.vboIndex));
                    GL15.glBufferData(ARBVertexBufferObject.GL_ARRAY_BUFFER_ARB, TmtTessellator.byteBuffer, GL15.GL_STREAM_DRAW);
                }
                ...
                if(this.hasTexture)
                {
                    if(TmtTessellator.useVBO)
                    {
                        GL11.glTexCoordPointer(4, GL11.GL_FLOAT, 40, 12);
                    }
                    else
                    {
                        TmtTessellator.floatBuffer.position(3);
                        GL11.glTexCoordPointer(4, GL11.GL_FLOAT, 40, TmtTessellator.floatBuffer);
                    }
                ...
                if(this.hasBrightness)
                {
                    ...
                    if(TmtTessellator.useVBO)
                    {
                        GL11.glTexCoordPointer(2, GL11.GL_SHORT, 40, 36);
                    }
                    else
                    {
                        TmtTessellator.shortBuffer.position(18);
                        GL11.glTexCoordPointer(2, GL11.GL_FLOAT, 40, TmtTessellator.shortBuffer);
                    }
                ...
                if(this.hasColor)
                {
                    if(TmtTessellator.useVBO)
                    {
                        GL11.glColorPointer(4, GL11.GL_UNSIGNED_BYTE, 40, 28);
                    }
                    else
                    {
                        TmtTessellator.byteBuffer.position(28);
                        GL11.glColorPointer(4, GL11.GL_UNSIGNED_BYTE, 40, TmtTessellator.byteBuffer);
                    }
                ...
                if(this.hasNormals)
                {
                    ...
                    else
                    {
                        ...
                        GL11.glNormalPointer(40, GL11.GL_FLOAT, TmtTessellator.byteBuffer);
                    }
                ...
                else
                {
                    ...
                    GL11.glVertexPointer(3, GL11.GL_FLOAT, 40, TmtTessellator.floatBuffer);
                }
