...
public final class RenderGun
{
    ...
    private static void renderSpinningCocking(ModelGun model, GunAnimations animations, PoseStack poseStack)
    {
        ...
        float pumped = (animations.lastPumped + (animations.pumped - animations.lastPumped) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
    ...
    private static void renderSlideModels(ModelGun model, ItemStack stack, GunAnimations animations, AttachmentType slideAttachment, AttachmentType scopeAttachment, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, float scale, EnumRenderPass renderPass)
    {
        if (slideAttachment == null)
        {
            ...
            {
                poseStack.translate(-(animations.lastGunSlide + (animations.gunSlide - animations.lastGunSlide) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true)) * model.gunSlideDistance, 0F, 0F);
                poseStack.translate(-(1 - Math.abs(animations.lastCharged + (animations.charged - animations.lastCharged) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))) * model.chargeHandleDistance, 0F, 0F);
            }
            ...
            {
                poseStack.pushPose();
                poseStack.translate(-(animations.lastGunSlide + (animations.gunSlide - animations.lastGunSlide) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true)) * model.altgunSlideDistance, 0F, 0F);
    ...
    private static void renderPumpAction(ModelGun model, GunAnimations animations, AttachmentType pumpAttachment, AttachmentType gripAttachment, AttachmentType gadgetAttachment, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, float scale, EnumRenderPass renderPass)
    {
        ...
        {
            poseStack.pushPose();
            poseStack.translate(-(1 - Math.abs(animations.lastPumped + (animations.pumped - animations.lastPumped) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))) * model.pumpHandleDistance, 0F, 0F);
    ...
    {
        poseStack.pushPose();
        poseStack.translate(-(1 - Math.abs(animations.lastPumped + (animations.pumped - animations.lastPumped) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))) * model.boltCycleDistance, 0F, 0F);
        poseStack.translate(model.boltRotationOffset.x, model.boltRotationOffset.y, model.boltRotationOffset.z);
        poseStack.mulPose(Axis.XP.rotationDegrees(-(1 - Math.abs(animations.lastPumped + (animations.pumped - animations.lastPumped) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))) * model.boltRotationAngle));
        poseStack.translate(-model.boltRotationOffset.x, -model.boltRotationOffset.y, -model.boltRotationOffset.z);
    ...
    private static void renderAmmo(ModelGun model, GunAnimations animations, ItemStack stack, int numRounds, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha, float scale, EnumRenderPass renderPass)
    {
        ...
        {
            // Calculate the amount of tilt required for the reloading animation
            float effectiveReloadAnimationProgress = animations.lastReloadAnimationProgress + (animations.reloadAnimationProgress - animations.lastReloadAnimationProgress) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true);
    ...
    private static void renderCasingEjection(ModelGun model, GunAnimations animations, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay)
    {
        ...
        if (casing != null)
        {
            float casingProg = (animations.lastCasingStage + (animations.casingStage - animations.lastCasingStage) * Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true)) / model.casingAnimTime;
            if (casingProg >= 1)
                casingProg = 0;
