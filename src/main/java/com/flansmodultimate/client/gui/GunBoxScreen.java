...
public class GunBoxScreen extends AbstractContainerScreen<GunBoxMenu>
{
    ...
    {
        GunBoxBlock block = menu.getBlock();
        if (block == null || block.getConfigType() == null)
            return;

        GunBoxType type = block.getConfigType();
    ...
    private GunBoxType.GunBoxPage getCurrentPage()
    {
        ...
        if (block == null || block.getConfigType() == null)
            return null;
        List<GunBoxType.GunBoxPage> pages = block.getConfigType().getGunPages();
        if (pages.isEmpty())
            return null;
    ...
    private int getPageCount()
    {
        ...
        if (block == null || block.getConfigType() == null)
            return 0;
        return block.getConfigType().getGunPages().size();
    }
