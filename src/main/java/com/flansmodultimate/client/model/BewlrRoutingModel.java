    ...
    public BewlrRoutingModel(BakedModel original)
    {
        ...
        wrappedOverrides = new ItemOverrides()
        {
            ...
            public BakedModel resolve(@NotNull BakedModel model, @NotNull ItemStack stack, @Nullable ClientLevel level, @Nullable LivingEntity entity, int seed)
            {
                ...
                routingModel.hasCustomModel = (stack.getItem() instanceof ICustomRendereredItem<?> customRendererItem)
                    && (ModelCache.getOrLoadTypeModel(customRendererItem.getConfigType()) != null
                    || CustomItemRenderers.get((Item) customRendererItem) != null);
                return routingModel;
            }
        };
    }
