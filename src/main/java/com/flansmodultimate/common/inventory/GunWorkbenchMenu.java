...
public class GunWorkbenchMenu extends AbstractContainerMenu
{
    ...
    {
        CustomData gunCustomData = gunStack.get(DataComponents.CUSTOM_DATA);
        CompoundTag gunTag = gunCustomData != null ? gunCustomData.copyTag() : new CompoundTag();
        CompoundTag attachments = gunTag.getCompound(GunItem.NBT_ATTACHMENTS);

        // specific 1..8
        for (int i = 0; i < SPECIFIC_ATTACHMENT_SLOTS; i++)
        {
            ItemStack att = loadAttachmentTag(attachments, SPECIFIC_TAGS[i]);
        ...
        for (int i = 0; i < GENERIC_ATTACHMENT_SLOTS; i++)
        {
            ItemStack att = loadAttachmentTag(attachments, GunItem.NBT_GENERIC + i);
    ...
    private ItemStack loadAttachmentTag(CompoundTag attachments, String key)
    {
        CompoundTag tag = attachments.getCompound(key);
        if (tag.isEmpty())
            return ItemStack.EMPTY;
        if (tag.contains("flans_attachment"))
        {
            ResourceLocation itemId = ResourceLocation.parse(tag.getString("flans_attachment"));
            Item item = BuiltInRegistries.ITEM.get(itemId);
            if (item != null && item instanceof AttachmentItem)
    ...
    {
        CustomData gunCustomData = gunStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        CompoundTag gunTag = gunCustomData.copyTag();
    ...
    private void writeAttachmentTag(CompoundTag attachments, ItemStack stack, String name)
    {
        if (!stack.isEmpty() && stack.getItem() instanceof AttachmentItem)
        {
            ...
            if (registryName != null)
                t.putString("flans_attachment", registryName.toString());
            attachments.put(name, t);
        }
        else
        {
            attachments.put(name, new CompoundTag());
        }
    }
