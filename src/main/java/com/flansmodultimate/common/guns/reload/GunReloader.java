...
public record GunReloader(GunItem item)
{
    ...
    public boolean reload(Level level, ServerPlayer player, PlayerData data, ItemStack gunStack, InteractionHand hand, boolean forceReload, boolean instabuild, boolean combineAmmoOnReload, boolean ammoToUpperInventory, UUID reloadSoundUUID)
    {
        ...
        if (ModCommonConfig.get().enableDigitalAmmoSystem())
        {
            ...
            {
                long ticks = (long) Math.ceil(item.getActualReloadTime(gunStack, level.registryAccess()));
    ...
    private List<ReloadPlan> computeDigitalAmmoPlans(ItemStack gunStack, List<ShootableType> allowedAmmoTypes, boolean forceReload, HolderLookup.Provider registryAccess)
    {
        ...
        {
            ItemStack current = item.getAmmoItemStack(gunStack, i, registryAccess);
    ...
    private List<ReloadPlan> computePlans(ItemStack gunStack, Container inventory, List<ShootableType> allowedAmmoTypes, String preferredAmmo, boolean forceReload, HolderLookup.Provider registryAccess)
    {
        int ammoSlots = item.getConfigType().getNumAmmoItemsInGun(gunStack);
        List<ReloadPlan> plans = new ArrayList<>(ammoSlots);
