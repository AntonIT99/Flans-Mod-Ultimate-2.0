...
public class GunType extends PaintableType implements IScope
{
    ...
    {
        checkForTags(gun);
        var customData = gun.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
    ...
    public ItemStack getAttachmentItemStack(ItemStack gun, String name)
    {
        ...
        if (stackTag.contains("flans_attachment"))
        {
            ResourceLocation itemId = ResourceLocation.parse(stackTag.getString("flans_attachment"));
            ItemStack result = new ItemStack(BuiltInRegistries.ITEM.get(itemId), 1);
            if (!result.isEmpty() && result.getItem() instanceof AttachmentItem)
                return result;
            return ItemStack.EMPTY;
        }
        ...
        return net.minecraft.world.item.ItemStack.parse(lookup, stackTag).orElse(ItemStack.EMPTY);
    }
    ...
    public EnumFireMode getFireMode(@Nullable ItemStack stack)
    {
        if (stack != null)
        {
            ...
            var customData = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
            CompoundTag tag = customData.copyTag();
            if (!tag.isEmpty() && tag.contains(GunItem.NBT_GUN_MODE))
            {
