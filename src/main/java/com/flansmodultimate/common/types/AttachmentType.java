...
public class AttachmentType extends PaintableType implements IScope
{
    ...
    public static AttachmentType getFromNBT(CompoundTag tags) {
        if (tags.isEmpty())
            return null;
        if (tags.contains("flans_attachment")) {
            String fullId = tags.getString("flans_attachment");
            String shortName = fullId;
            int colon = fullId.indexOf(':');
            if (colon >= 0)
                shortName = fullId.substring(colon + 1);
            return getAttachment(shortName);
        }
        ...
        ItemStack stack = net.minecraft.world.item.ItemStack.parse(lookup, tags).orElse(ItemStack.EMPTY);
