...
public class PacketBlockHitEffect implements IClientPacket
{
    ...
    public void handleClientSide(@NotNull Player player, @NotNull Level level)
    {
        ...
        {
            double px1 = x + (level.getRandom().nextFloat() - 0.3) * bbWidth * 0.05;
            double py1 = y + (level.getRandom().nextFloat() - 0.3) * bbWidth * 0.05;
            double pz1 = z + (level.getRandom().nextFloat() - 0.3) * bbWidth * 0.05;

            double vx1 = -motion.x * (0.0011 + level.getRandom().nextGaussian() * 0.008) * velocityFactor;
            double vy1 = Math.abs(0.305 + level.getRandom().nextDouble() * 0.125) * velocityFactor;
            double vz1 = -motion.z * (0.0011 + level.getRandom().nextGaussian() * 0.008) * velocityFactor;

            double px2 = x + (level.getRandom().nextFloat() - 0.6) * bbWidth * 0.75;
            double py2 = y + (level.getRandom().nextFloat() - 0.6) * bbWidth * 0.75;
            double pz2 = z + (level.getRandom().nextFloat() - 0.6) * bbWidth * 0.75;

            double vx2 = -motion.x * (0.415 + level.getRandom().nextGaussian() * 0.1) * velocityFactor;
            double vy2 = -motion.y * (0.425 + Math.abs(level.getRandom().nextGaussian() * 0.1)) * velocityFactor;
            double vz2 = -motion.z * (0.415 + level.getRandom().nextGaussian() * 0.1) * velocityFactor;
