...
| `src/main/java/com/flansmodultimate/CreativeTabs.java`; `platform/creative/CreativeTabPlatform.java` | Creative tab definitions and ordering are shared. `FlansMod` passes a registration callback so each loader keeps its own deferred registry. The platform helper reads driveable stack data with the tab display's holder provider (ignored on 1.20.1) and supplies the item registry key for sorting. |
| `src/main/java/com/flansmodultimate/FlansMod.java`, `apocalyse/ApocalypseContent.java` and registry lookups | Both branches create deferred registers from vanilla `Registries` keys and look entries up through `BuiltInRegistries`; only Forge/NeoForge-owned registries such as fluid types keep a loader key. `RegistryObject` versus `DeferredHolder` remains the holder type difference. |
