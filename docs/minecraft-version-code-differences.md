...
| Class or location | Source API and destination adaptation |
| --- | --- |
| `src/main/java/com/flansmodultimate/common/recipe/RecipeJsonGenerator.java`; `src/main/java/com/flansmodultimate/ContentManager.java` | Shared legacy `.txt` definitions are converted to 1.21.1 JSON under `recipe/`, not 1.20.1 `recipes/`. Crafting outputs use `result.id`; smelting outputs are objects with `id`; ingredients still use `item` or `tag`. The generator migrates old preprocessed recipe JSON when loading packs. Minecraft 1.21.1 rejects an output `count` above the registered item's stack limit. The generator caps counts and detects oversized preprocessed recipes for regeneration. Shootable parsing can raise `MaxStackSize` to `RecipeOutput`; durability-based fuel parts, guns, and tools remain single items. A legacy recipe requesting multiple unstackable items therefore yields one item. |
| `src/main/resources/data/*/recipe/`; `src/*/resources/flans_content/*/data/*/{recipes,recipe}/` | Minecraft 1.21.1 loads recipes from singular `recipe/`, while 1.20.1 loads plural `recipes/`. Source content packs ship both directories in both branches; Minecraft ignores the inactive directory, though the 1.21.1 mod can migrate missing singular files from it. Each directory retains its version-specific output schema and counts. Main mod recipes remain in the version's active directory. Java compilation alone cannot verify these resources. |
