...
| `src/main/java/com/flansmodultimate/FlansMod.java`; `client/CreativeTabs.java` | Both branches expose registered items, blocks, and menus as `Supplier<? extends T>` and creative-tab inputs as supplier lists. Forge `RegistryObject` and NeoForge `DeferredHolder` stay at the registration boundary; menu creation still uses each loader's native factory. This keeps callers shared without changing registration timing. |
