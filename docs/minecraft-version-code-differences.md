...
| `src/main/java/com/flansmodultimate/platform/PlatformEnvironment.java`; `src/packsmanager/java/com/flansmodultimate/packsmanager/platform/PlatformEnvironment.java` | Runtime side and production checks use a small API in each independent source set. Forge and NeoForge retain their own `FMLEnvironment` imports there; annotation values and Forge's client packet `DistExecutor` call still use their required loader APIs. |
| `src/main/java/com/flansmodultimate/FlansMod.java`; `CreativeTabs.java`; `platform/creative/CreativeTabPlatform.java` | Both branches expose registered items, blocks, and menus as `Supplier<? extends T>` and creative-tab inputs as supplier lists. `FlansMod` passes a registration callback so each loader keeps its own deferred registry. Creative tab definitions and ordering are shared; the platform helper reads driveable stack data with the tab display's holder provider on 1.21.1 and without one on 1.20.1, and supplies each loader's item registry key for sorting. |
| `src/main/java/com/flansmodultimate/config/{ModCommonConfig,ModClientConfig,ModApocalypseConfig}.java` | Read-only private config value fields use `Supplier<T>` on both branches. Forge and NeoForge builders, specs, and public or mutable config values retain their native types, so registration and updates still follow the respective loader API. |
...
| `src/main/java/com/flansmodultimate/network/PacketBuffer.java`; `network/{IPacket.java,client/*,server/*}` | Packet classes on both branches encode and decode through `PacketBuffer`. The Forge wrapper extends `FriendlyByteBuf`; the NeoForge wrapper extends `RegistryFriendlyByteBuf` and retains registry access for item-stack codecs. `PacketHandler` constructs the appropriate wrapper at the transport boundary. Packet payload logic can remain shared while each loader keeps its native registration and dispatch. |

### State, events, and menus

| Class or location | Source API and destination adaptation |
| --- | --- |
| `src/main/java/com/flansmodultimate/platform/item/ItemStackData.java`; `common/driveables/DriveableData.java` | Both branches use the same `ItemStackData` operations for custom stack data. Forge backs them with the mutable stack tag; NeoForge uses item data components and passes `HolderLookup.Provider` when saving or parsing stacks, including nested driveable inventory and magazines. `copy` returns detached data on both branches, so callers write edited data back with `set`. |
...
| Class or location | Source API and destination adaptation |
| --- | --- |
| `src/main/java/com/flansmodultimate/client/model/ModelBase.java`; `src/officialpacks/java/com/flansmod/client/model/yeolde/ModelArrow.java` | Forge 1.20.1 emits a vertex through the fluent `vertex(...).color(...).uv(...).overlayCoords(...).uv2(...).normal(...).endVertex()` chain; NeoForge 1.21.1 uses `addVertex(...).setColor(...).setUv(...).setOverlay(...).setLight(...).setNormal(...)`. The version-specific call lives in the protected `ModelBase.addVertex` helper, so `ModelArrow` keeps the same geometry and tint calls on both branches. |
